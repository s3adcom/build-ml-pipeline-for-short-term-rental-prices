...
def go(args):
    ...
    logger.info("Dataset price outliers removal outside range: %s-%s", args.min_price, args.max_price)

    dataframe['last_review'] = pd.to_datetime(dataframe['last_review'])
    logger.info("Dataset last_review data type fix")

    idx = dataframe['latitude'].between(40.5, 41.2) & dataframe['longitude'].between(-74.25, -73.50)
    dataframe = dataframe[idx].copy()
    logger.info("Dataset latitude & longitude selected")

    tmp_artifact_path = os.path.join(os.getcwd(), args.output_artifact)
